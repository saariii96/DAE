...
# Top50 - Funktion zur Nebenwirkungsextraktion ---------------------------------------
...
# Top100 - Funktion zur Nebenwirkungsextraktion ---------------------------------------
get_sideeffects <- function(drug_name, limit = 200) {
  base_url <- "https://api.fda.gov/drug/event.json"
  query <- list(
    search = paste0("patient.drug.medicinalproduct:", drug_name),
    limit = limit
...
# Kombinieren und speichern
sideeffects_df <- do.call(rbind, sideeffects_list)
write.csv(sideeffects_df, "02-data/sideeffects_top100.csv", row.names = FALSE)
